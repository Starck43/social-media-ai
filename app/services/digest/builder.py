...
async def build_and_publish(period: str = "day", schedule_id: int | None = None) -> dict[str, Any]:
	...
	try:
		...
		if not results:
			await runs.update_by_id(
				run.id, status="skipped", content=text, error="No digest channels configured"
			)
			return {"status": "skipped", "reason": "no_channels", "text": text}

		ok = all(r.get("success") for r in results.values())
		message_id = next(
			(str(r.get("message_id")) for r in results.values() if r.get("message_id")), None
		)
