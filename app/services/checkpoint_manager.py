...
class CheckpointManager:
	...
	def should_collect(
		...
		last_check = cast(datetime, source.last_checked)
	...
	def get_collection_params(source: Source) -> dict[str, Any]:
		"""
		Get platform-specific collection parameters for incremental mode only.
		...
		# Only apply time filter for incremental mode
		if source.params.get('incremental_mode') and source.last_checked:
			params["since"] = source.last_checked

		# Add platform-specific cursors from source.params
		if source.params.get('incremental_mode') and source.params:
			# VK offset
	...
	@staticmethod
	async def prepare_for_incremental_collection(source: Source) -> Source:
		"""
		Prepare source for incremental background collection
		"""
		if not source.params:
			source.params = {}

		source.params['incremental_mode'] = True
		return source

	@staticmethod
	async def prepare_for_full_collection(source: Source) -> Source:
		"""
		Prepare source for full CLI collection
		"""
		if source.params:
			source.params.pop('incremental_mode', None)
		return source
