...
class DatabaseEnum(Enum):
    """
    Typed mixin for DB-backed enums.

    Provides static declarations of helpers attached at runtime by
    @database_enum so IDEs/type checkers can resolve `cls.sa_column()`,
    `cls.sa_enum()` and `cls.get_db_values()`.
    """

    @classmethod
    def get_db_values(cls, store_as_name: bool = False) -> list[str]:
        """Return DB values for enum members (names or db_values)."""
    ...
    def sa_column(
        ...
        **kwargs: Any,
        ...
        """Create a mapped_column for this enum type."""
        raise NotImplementedError  # attached by @database_enum
...
def database_enum(enum_class: Type[E]) -> Type[E]:
    ...
    def sa_column(cls, default=None, nullable=False, type_name: str = None, store_as_name: bool = False, **kwargs):
        ...
        """
        return mapped_column(cls.sa_enum(type_name, store_as_name), default=default, nullable=nullable, **kwargs)

    # Присваиваем методы как classmethod
